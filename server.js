...
app.get('/team_stats', function(req, res) {
  var games = 'SELECT * FROM football_games;';//select all games
  var games_wins = 'SELECT COUNT(*) FROM football_games WHERE home_score>visitor_score;';//count all the games where cu won
  var games_losses = 'SELECT COUNT(*) FROM football_games WHERE home_score<=visitor_score;';//count all the games where cu lost

  db.task('get-everything', task => {
        return task.batch([
            task.any(games),
            task.any(games_wins),
            task.any(games_losses)
        ]);
    })
    .then(data => {
    	res.render('pages/team_stats',{
				my_title: "Team Stats",
				games: data[0],
        games_wins: data[1][0].count,
        games_losses: data[2][0].count,
			})
    })
    .catch(error => {
        // display error message in case an error
